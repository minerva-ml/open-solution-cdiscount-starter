...
from utils import register_action, neptune_post_pipeline_score, logger, timeit, safe_sample, registered_actions, \
...
def evaluate_pipeline(args):
	...
	logger.info('accuracy on valid {0:.4f}'.format(accuracy_score(y_true, y_pred)))
...
def sample(args):
	...
	logger.info('train size: {}'.format(meta_train.shape))
	...
	logger.info('sampled train size: {}'.format(meta_train_sampled.shape))
